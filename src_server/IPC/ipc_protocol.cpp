...
namespace yancy
{
    ...
    {
        m_messageQueue.reset(new MessageQueue<MsgInfo*>());
    }
    ...
    void IpcProtocol::push_queue(MsgInfo* msgInfo)
    {
        m_messageQueue->push(msgInfo);
    }
    IpcProtocol::MsgInfo* IpcProtocol::pop_queue()
    {
        IpcProtocol::MsgInfo* msgInfo = nullptr;
        m_messageQueue->wait(msgInfo);  //等待队列中有数据
        return msgInfo;
    }
}